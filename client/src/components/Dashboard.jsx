...
function Dashboard({ refresh }) {
  ...
  const { data: dashRaw,    isLoading: l1 } = useQuery({
    ...
    queryFn:  () => API.get("/api/dashboard", { params, headers }).then(r => r.data),
  ...
  const { data: coursesRaw, isLoading: l2 } = useQuery({
    ...
    queryFn:  () => API.get("/api/dashboard/courses", { params, headers }).then(r => r.data),
    enabled:  !needsMonthSelection,
    staleTime: 5 * 60 * 1000,
  });
  ...
  const { data: renewalsRaw, isLoading: l3 } = useQuery({
    ...
    queryFn:  () => API.get("/api/dashboard/renewals-due", { headers }).then(r => r.data),
    staleTime: 5 * 60 * 1000,
  });

  const { data: paymentsRaw, isLoading: l4 } = useQuery({
    queryKey: ["dashboard-payments", paymentFilter],
    queryFn:  () => API.get("/api/dashboard/recent-payments", { params: { days: paymentFilter }, headers }).then(r => r.data),
    staleTime: 5 * 60 * 1000,
  });
